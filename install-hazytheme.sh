#!/bin/bash
set -e
echo "🌫️ HazyTheme One-Click Installer for Live Panels"

# Check location
if [ ! -f "artisan" ]; then
    echo "❌ Error: Run this from your Pterodactyl root folder."
    return 1 2>/dev/null
fi

echo "📦 Updating dependencies..."
sed -i '/"dependencies": {/a \        "lucide-react": "^0.263.1",' package.json
sed -i '/"dependencies": {/a \        "react-circular-progressbar": "^2.1.0",' package.json
...
# [Self-Extracting content starts here]
# I will use a clever way to write all the files.
...
:root {
    ...
    --hazy-bg: #0f172a;
    --hazy-text: #f8fafc;
    --hazy-glass-border: rgba(255, 255, 255, 0.1);
}
...
# [Patching logic]
echo "🛠️ Patching Core..."
# Nav bar Admin
sed -i "/admin.settings.advanced/a \                    <li @if(\$activeTab === 'hazytheme')class=\"active\"@endif><a href=\"{{ route('admin.settings.hazytheme') }}\">HazyTheme</a></li>" resources/views/partials/admin/settings/nav.blade.php
# Routes Admin
...
echo "🚀 Starting Production Build (this will take a while)..."
...
echo "✅ HazyTheme is now applied! Refresh your browser."

# Writing HazySidebar.tsx
...
# Writing App.tsx (Modified version)
...
# Swap NavigationBar for HazyNavbar in Routers
...
@section('content-header')
    ...
    <ol class="breadcrumb"><li><a href="{{ route('admin.index') }}">Admin</a></li><li class="active">Settings</li><li class="active">HazyTheme</li></ol>
@endsection
@section('content')
    @yield('settings::nav')
    <div class="row"><div class="col-xs-12"><div class="box"><div class="box-header with-border"><h3 class="box-title">Theme Configuration</h3></div>
                <form action="{{ route('admin.settings.hazytheme') }}" method="POST">
                    <div class="box-body"><div class="row">
                            <div class="form-group col-md-4"><label class="control-label">Primary Accent Color</label><div><input type="color" class="form-control" name="hazytheme::primary_color" value="{{ old('hazytheme::primary_color', $hazy_primary) }}" /><p class="text-muted"><small>Choose the highlight color.</small></p></div></div>
                            <div class="form-group col-md-4"><label class="control-label">Animated Backgrounds</label><div><select name="hazytheme::animation" class="form-control"><option value="true" @if($hazy_animation === 'true') selected @endif>Enabled</option><option value="false" @if($hazy_animation === 'false') selected @endif>Disabled</option></select></div></div>
                            <div class="form-group col-md-4"><label class="control-label">Sidebar Power Buttons</label><div><select name="hazytheme::sidebar_power" class="form-control"><option value="true" @if($hazy_sidebar_power === 'true') selected @endif>Enabled</option><option value="false" @if($hazy_sidebar_power === 'false') selected @endif>Disabled</option></select></div></div>
        ...
        <nav className='sticky top-0 w-full h-16 glass backdrop-blur-md border-b border-white/10 z-40 px-6 flex items-center justify-between'>
            ...
            <div className='flex items-center space-x-6'>
                <motion.div animate={{ width: searchFocused ? 400 : 200 }} className='relative group'>
                    <Search className='absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 transition-colors' size={18} style={{ color: searchFocused ? primaryColor : undefined }} />
                ...
                <div className='relative cursor-pointer text-slate-400 hover:text-violet-400 transition-colors' style={{ color: searchFocused ? primaryColor : undefined }}>
                    <Bell size={22} /><span className='absolute -top-1 -right-1 w-4 h-4 text-white text-[10px] flex items-center justify-center rounded-full border-2 border-slate-900 font-bold' style={{ backgroundColor: primaryColor }}>3</span>
                </div>
            </div>
        </nav>
    );
...
const HazyServerCard = ({ server }: any) => {
    ...
    return (
        <motion.div whileHover={{ scale: 1.02, y: -5 }} className='group glass rounded-3xl p-6 border border-white/10 hover:border-violet-500/50 transition-all duration-300' style={{ borderColor: `${primaryColor}33` } as any}>
            <Link to={`/server/${server.id}`} className='block'>
                <div className='flex justify-between mb-8'>
                    <div className='flex items-center space-x-4'>
                        <div className='p-4 bg-violet-500/10 rounded-2xl transition-all duration-500' style={{ backgroundColor: `${primaryColor}1a`, color: primaryColor }}><ServerIcon size={28} /></div>
                ...
                <div className='grid grid-cols-3 gap-6'>
                    {[{l:'CPU',v:cpu,t:stats?.cpuUsagePercent.toFixed(0)+'%'},{l:'RAM',v:ram,t:stats?bytesToString(stats.memoryUsageInBytes):'0MB'},{l:'Disk',v:20,t:stats?bytesToString(stats.diskUsageInBytes):'0MB'}].map((x,i)=>(
                        <div key={i} className='flex flex-col items-center'><div className='w-14 h-14 mb-2'><CircularProgressbar value={x.v} strokeWidth={10} styles={buildStyles({pathColor:primaryColor,trailColor:'rgba(255,255,255,0.03)'})}/></div><span className='text-[10px] text-slate-500 font-bold uppercase'>{x.l}</span><span className='text-xs text-slate-300 font-medium mt-1'>{x.t}</span></div>
                    ))}
                </div>
            </Link>
        </motion.div>
...
const HazyButton = ({ variant = 'primary', isLoading, children, className = '', ...props }: any) => {
    const primaryColor = useStoreState((state: any) => state.settings.data?.hazytheme?.primary_color || '#6366f1');
    const v: any = { primary: 'text-white shadow-lg', secondary: 'bg-slate-800 hover:bg-slate-700 text-slate-200 border border-white/5', danger: 'bg-red-600 hover:bg-red-500 text-white shadow-red-900/20', glass: 'glass hover:bg-white/10 text-white border-white/10' };
    const style = variant === 'primary' ? { backgroundColor: primaryColor, boxShadow: `0 10px 15px -3px ${primaryColor}33` } : {};
    return (<motion.button whileTap={{ scale: 0.96 }} className={`relative px-6 py-2.5 rounded-xl font-semibold transition-all flex items-center justify-center space-x-2 disabled:opacity-50 ${v[variant] || v.primary} ${className}`} style={style} disabled={isLoading} {...props}>{isLoading && <Loader2 size={18} className='animate-spin' />}<span className={isLoading ? 'opacity-0' : 'opacity-100'}>{children}</span></motion.button>);
};
export default HazyButton;
EOF

# Writing HazyResourceGraph
...
const HazyResourceGraph = ({ data }: { data: { cpu: number; ram: number; disk: number } }) => {
    const primaryColor = useStoreState((state: any) => state.settings.data?.hazytheme?.primary_color || '#6366f1');
    return (<div className='space-y-4 w-full'>{(['cpu', 'ram', 'disk'] as const).map((k) => (<div key={k} className='space-y-1'><div className='flex justify-between text-[10px] font-bold text-slate-500 uppercase tracking-widest'><span>{k}</span><span>{data[k]?.toFixed(1)}%</span></div><div className='h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/5'><motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(data[k] || 0, 100)}%` }} transition={{ duration: 1, ease: 'easeOut' }} className='h-full shadow-[0_0_10px_rgba(167,139,250,0.5)]' style={{ backgroundColor: primaryColor }} /></div></div>))}</div>);
};
...
echo "🏗️ Installing JS dependencies (yarn install)..."
yarn install --ignore-engines

echo "🚀 Building production assets (yarn build:production)..."
...
echo "✨ HazyTheme Installation Complete! Refresh your browser with Ctrl+F5."
