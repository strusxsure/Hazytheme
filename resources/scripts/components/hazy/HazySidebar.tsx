...
const HazySidebar = () => {
    const [collapsed, setCollapsed] = useState(() => {
        const saved = localStorage.getItem('hazy_sidebar_collapsed');
    ...
    return (
        ...
        >
            ...
            <nav className="flex-1 px-2 space-y-2">
                <NavLink to="/" exact activeClassName="text-violet-400 bg-violet-500/10 !border-l-4 border-violet-400" className="flex items-center p-3 text-slate-400 hover:text-violet-400 rounded-xl transition-all border-l-4 border-transparent">
                    <LayoutDashboard size={24} className="min-w-[24px]" />
                    {!collapsed && <span className="ml-4 font-medium">Dashboard</span>}
                </NavLink>
                <NavLink to="/account" activeClassName="text-violet-400 bg-violet-500/10 !border-l-4 border-violet-400" className="flex items-center p-3 text-slate-400 hover:text-violet-400 rounded-xl transition-all border-l-4 border-transparent">
                    <User size={24} className="min-w-[24px]" />
                    {!collapsed && <span className="ml-4 font-medium">Account</span>}
                </NavLink>
                {rootAdmin && (
                    <a href="/admin" className="flex items-center p-3 text-slate-400 hover:text-violet-400 rounded-xl transition-all border-l-4 border-transparent">
            ...
            <div className="p-4 mt-auto border-t border-white/5 space-y-4">
                ...
                <button onClick={onLogout} className="w-full flex items-center p-3 text-red-400 hover:bg-red-500/10 rounded-xl transition-all">
